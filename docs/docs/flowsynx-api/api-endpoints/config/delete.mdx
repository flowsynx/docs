...
<Tabs groupId="httpApi">
  ...
  ```
  curl -X DELETE -H "Content-Type: application/json" -d '{ "name": "azblob" }' http://localhost:5860/config/delete
  ```
  </TabItem>
  <TabItem value="powershellHttpApi" label="HTTP API (PowerShell)">
  ```
  Invoke-RestMethod -Method Delete -ContentType 'application/json' -Body '{ "name": "azblob" }' -Uri 'http://localhost:5860/config/delete'
  ```
  </TabItem>
</Tabs>
