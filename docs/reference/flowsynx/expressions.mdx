...
description: FlowSynx expression system for dynamic access to variables, task outputs, secrets, nested data, arithmetic, conditionals, boolean logic, and functional aggregation.
---

# FlowSynx Expression Syntax

FlowSynx provides a powerful expression system for dynamically resolving workflow data at runtime, including:
- Variables and task outputs
- Secrets
...
- If the entire value is a single `$[...]` block, the resolved object is returned directly (e.g., an array, `JObject`, or complex CLR type).
- If embedded inside a larger string, the resolved value is converted to a string and concatenated.
...
| Secrets   | `$[Secrets('Name')]`       | Secret value resolved via default secret provider (if set).  |

Keys with spaces or special characters should use single quotes: `$[Variables('My Custom Key')]`.

### Outputs as PluginContext

The `Outputs('TaskName')` reference typically resolves to a `PluginContext` object with:

| Property         | Description                                                                 |
|----------------- |----------------------------------------------------------------------------- |
| `Id`             | The unique identifier of the plugin context.                                 |
| `SourceType`     | The source type that generated the output.                                   |
| `Format`         | The format of the output, if applicable.                                     |
| `Metadata`       | A dictionary containing metadata as key-value pairs (case-insensitive).      |
...
If a default secret provider is configured, `Secrets('Name')` resolves secret values:

```
$[Secrets('ApiKey')] 
$[Contains(Secrets('AllowedHosts'), 'example.com')]
```

If no default provider is available, `Secrets(...)` is unsupported.
...
| `Count`   | `Count(collection)` or `Count(arg1,...)`    | Single enumerable → element count; otherwise number of arguments.                        |
| `Contains`| `Contains(container, value)`                | Case-insensitive search (strings, enumerables, dictionaries, or direct value comparison).|
| `Length`  | `Length(value)`                             | Returns length of strings, arrays, lists, JArrays, or enumerable count.                  |
| `Guid`    | `Guid()`                                    | Returns a new GUID string.                                                               |
| `Now`     | `Now()`                                     | Returns current local date/time.                                                         |
| `NowUtc`  | `NowUtc()`                                  | Returns current UTC date/time.                                                           |
...
$[Length(Outputs('Task').Content)] 
$[IsNull(Outputs('Task').StructuredData) ? 'none' : 'has-data']
```

:::note
- When a function appears inside arithmetic, it is evaluated and replaced by its string representation for numeric computation.
- A standalone function inside `$[...]` returns its native result (number, string, bool).
:::
...
```
Variables: { "Base": "/data/out" } 
Outputs: { "Export": { "Id": "report.csv" } }
```
...
Contextual behavior:
- In arithmetic-only contexts, unresolved lookups are coerced to `"0"`.
- Direct top-level lookups that cannot be resolved may throw an exception.
...
- Avoid over-nesting; store intermediate values in variables or outputs where helpful.
- Use `IsNull(...)` to guard against missing data before navigation.

## Summary

FlowSynx Expressions support:
- Dynamic access to variables, task outputs, and secrets
- Nested property and array traversal
- Functional aggregation (Min/Max/Sum/Avg/Count/Contains/Length)
- Utility functions (`Guid`, `Now`, `NowUtc`, `IsNull`)
