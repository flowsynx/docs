...
| `AdditionalOptions` | object (key-value) | Optional PostgreSQL-specific settings. Each key represents a valid Npgsql connection string option. Common options include: <br /> - `SslMode` — Controls SSL/TLS usage (`Disable`, `Prefer`, `Require`, etc.)<br />- `Pooling` — Enables or disables connection pooling (`true` or `false`)<br />- Any other valid Npgsql option can be added. |
