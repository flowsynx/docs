...
This api will delete entity from specific storage path.

## DeleteFile Api options
...
<table>
  ...
  <tr>
    ...
    <td>The entity's path that are going to be deleted</td>
  </tr>
</table>

## Calling DeleteFile Api
...
<table class="table">
  ...
  <tr>
    <td>http://localhost:5860/storage/deletefile</td>
...
<Tabs groupId="httpApi">
  ...
  ```
  curl -X POST -H "Content-Type: application/json" -d '{ "path": "/path/testfile.txt" }' http://localhost:5860/storage/deletefile
  ```
  </TabItem>
  <TabItem value="powershellHttpApi" label="HTTP API (PowerShell)">
  ```
  Invoke-RestMethod -Method Post -ContentType 'application/json' -Body '{ "path": "C:/path/testfile.txt" }' -Uri 'http://localhost:5860/storage/deletefile'
  ```
  </TabItem>
</Tabs>
